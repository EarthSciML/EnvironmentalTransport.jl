...
function BoundaryLayerMixingKC(; name = :BoundaryLayerMixingKC)
    @constants κ = 0.4 [unit = u"1", description = "von Kármán constant"]
    ...
    one_m3zL = max(one - 3 * z_agl / L_eff, 1.0e-6)              # (1 - 3 z/L) for MOST; guard >0  (ARL-224 Eq. 92)
    is_unst = Lmo < m0                                      # unstable if L<0
    is_sfc = ζ_clip < 0.1                                 # crude surface layer/mixed layer split
    ...
    σw2_UML = wstar^2 * max(ζ_clip, 1.0e-6)^(2 / 3) * max(one - ζ_clip, 1.0e-6)^(2 / 3) *
        (one + 0.5 * R_inv^(2 / 3))
    ...
    eqs = [
        ...
        wprime_ref ~ ifelse(
            (lev <= lev_gnd + offset), (one - w_damp) * abs(wprime),
            ifelse(
                (lev >= lev_top - offset), -(one - w_damp) * abs(wprime),
                wprime
            )
