...
function BoundaryLayerMixingKC(; name=:BoundaryLayerMixingKC)
    ...
    @constants kms1e_8  = 1.0e-8  [unit = u"K*m/s", description = "Minimum kinematic heat flux"]
    ...
    @parameters TL_h        = 10800.0       [unit=u"s",             description = "Lagrangian time scale for horizontal-velocity"]
    @parameters TL_v_min    = 5.0           [unit=u"s",             description = "Minimum Lagrangian time scale for turbulence"]
    @parameters TL_v_max    = 1000.0        [unit=u"s",             description = "Maximum Lagrangian time scale for turbulence"]
    ...
    @parameters x_trans     = 0.01          [unit = u"1/m",         description = "lon radians per meter (1/δxδlon)"]
    @parameters y_trans     = 0.01          [unit = u"1/m",         description = "lat radians per meter (1/δyδlat)"]
    ...
    @parameters w_damp      = 1.0           [unit = u"1",           description = "Fraction of velocity lost on boundary impact (0.0 = perfect bounce, 1.0 = complete Absorption"]
    ...
    @variables σu(t)         [unit = u"m/s",    description = "Standard deviation of x component of turbulent velocity"]
    @variables σv(t)         [unit = u"m/s",    description = "Standard deviation of y component of turbulent velocity"]
    @variables TL_v(t)       [unit = u"s",      description = "Lagrangian time scale for vertical-velocity"]
    ...
    @variables wthv_eff(t)   [unit = u"K*m/s",  description = "Effective potential temperature flux"]
    ...
    eqs = [
        ...
        D(lon) ~ uprime_x * x_trans,
        D(lat) ~ uprime_y * y_trans,
        D(lev) ~ wprime_ref * z2lev
    ]
    
    System(eqs, t, 
        [zi, sigmaw, σu, σv, TL_v, wprime, uprime_x, uprime_y, Tv2m, rho, wth, wq, wthv, wthv_eff, Lmo, wstar, wprime_ref, lon, lat, lev],
        [κ, one, offset , m0, m1, ms0, kms0, kms1e_8, w_damp, S, PBLH, USTAR, HFLUX, EFLUX, PS, T2M, QV2M, z2lev, TL_h, TL_v_min, TL_v_max, Rd, cp, Lv, g, R_inv, KMIX0, TKEMIN, x_trans, y_trans, z_agl, lev_gnd, lev_top];
